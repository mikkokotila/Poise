...
// Only the native executable is replaced by default; opt-in Grok uses its
// installed login. All repository/database state stays under the test root.
...
const runtime = new ChatRuntime({
  ...
  adapters: { grok: host => createGrokAdapter({ ...host, async spawn(command, args, options) {
    ...
    const child = await host.spawn(live ? command : process.execPath,
      live ? args : [join(sourceRoot, 'tests/fixtures/chat/latency-agent.mjs')], options)
...
const record = await runtime.create({ agent: 'grok', model: 'grok-4.6-high', repo: 'fixture/latency', branch: { existing: 'main' }, title: 'Streaming latency' })
...
const server = createServer((req, res) => {
  void (async () => {
    ...
    if (path === '/__test__/timings') return json(res, { nativeFrames, spawnCount, live, sessionId: record.id })
