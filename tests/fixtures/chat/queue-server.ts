...
const runtime = new ChatRuntime({
  ...
  adapters: { grok: host => createGrokAdapter({ ...host, async spawn(command, args, options) {
    ...
    child.stdin!.write = ((chunk: any, ...rest: any[]) => {
      ...
      for (const line of lines) {
        try { const frame = JSON.parse(line); if (frame.method === 'session/prompt') nativeInputs.push(frame.params.prompt); if (frame.method === '_x.ai/interject') nativeSteers.push(frame.params.text) } catch { /* partial frame */ }
      }
