...
test('creates a session from the dialog with a pending entry before the server answers', async ({ page }) => {
  ...
  await expect(dialog.getByLabel('Model').locator('optgroup')).toHaveCount(6)
  await expect(dialog.getByLabel('Model').locator('option[value="jev-primitives"]')).toHaveText('JEV · Primitive builder')
