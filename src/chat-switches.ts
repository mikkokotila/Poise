...
export interface ChatSwitch { name: string, content: string, revision: number, updatedAt: string }
...
export function parseChatSwitches(value: unknown): ChatSwitches | null {
  ...
  if (!Number.isSafeInteger(data.revision) || data.revision < 0 || !Array.isArray(data.switches) || data.switches.length > SWITCH_LIMITS.count) return null
  ...
  for (const item of data.switches) {
    ...
    if (item.revision > data.revision || new TextEncoder().encode(item.content).byteLength > SWITCH_LIMITS.contentBytes) return null
    names.add(item.name)
  }
  if (new TextEncoder().encode(JSON.stringify(data)).byteLength > SWITCH_LIMITS.totalBytes) return null
  return data
}
