...
type ViewName = 'main' | 'stream' | 'swarm'
...
function loadView(): ViewName {
  try {
    ...
    if (v === 'flow' || v === 'trust') {
      // Flow + Trust were dropped — fall back to Main rather than stranding the user.
      localStorage.setItem(VIEW_KEY, 'main')
      return 'main'
    }
    if (v === 'main' || v === 'stream' || v === 'swarm') return v
  } catch { /* ignore */ }
  return 'main'
}
